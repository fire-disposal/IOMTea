...
export default function GoalsSettings() {
  ...
  const update = useCallback(<K extends keyof HealthGoals>(section: K, field: string, value: string) => {
    setGoals(prev => ({
      ...prev,
  ...
  return (
    <View className='goals-page'>
      <View className='goals-page__header'>
        <Text className='goals-page__back' onClick={() => Taro.navigateBack()}>← 返回</Text>
      ...
      <View className='goals-section'>
        ...
        <View className='goals-field'>
          ...
          <NumberInput value={String(goals.bloodGlucose.fastingMin)} onChange={v => update('bloodGlucose', 'fastingMin', v)} decimal />
        </View>
        <View className='goals-field'>
          <Text className='goals-field__label'>空腹血糖最高 (mmol/L)</Text>
          <NumberInput value={String(goals.bloodGlucose.fastingMax)} onChange={v => update('bloodGlucose', 'fastingMax', v)} decimal />
        </View>
        <View className='goals-field'>
          <Text className='goals-field__label'>餐后血糖最高 (mmol/L)</Text>
          <NumberInput value={String(goals.bloodGlucose.postprandialMax)} onChange={v => update('bloodGlucose', 'postprandialMax', v)} decimal />
        </View>
      </View>
      ...
      <View className='goals-section'>
        ...
        <View className='goals-field'>
          ...
          <NumberInput value={String(goals.weight.targetKg)} onChange={v => update('weight', 'targetKg', v)} decimal />
        </View>
        <View className='goals-field'>
          <Text className='goals-field__label'>最低体重 (kg)</Text>
          <NumberInput value={String(goals.weight.minKg)} onChange={v => update('weight', 'minKg', v)} decimal />
        </View>
        <View className='goals-field'>
          <Text className='goals-field__label'>最高体重 (kg)</Text>
          <NumberInput value={String(goals.weight.maxKg)} onChange={v => update('weight', 'maxKg', v)} decimal />
        </View>
      </View>
      ...
      <View className='goals-section'>
        ...
        <View className='goals-field'>
          ...
          <NumberInput value={String(goals.bloodPressure.systolicMax)} onChange={v => update('bloodPressure', 'systolicMax', v)} />
        </View>
        <View className='goals-field'>
          <Text className='goals-field__label'>舒张压上限 (mmHg)</Text>
          <NumberInput value={String(goals.bloodPressure.diastolicMax)} onChange={v => update('bloodPressure', 'diastolicMax', v)} />
        </View>
      </View>

      <Button className='goals-page__save' onClick={save}>保存</Button>
    </View>
  )
}
