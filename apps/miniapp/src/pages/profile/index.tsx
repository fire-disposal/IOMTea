...
export default function Profile() {
  ...
  const [user, setUser] = useState<{ displayName: string | null; username: string }>({
    displayName: null,
    username: '',
  })
  ...
  useEffect(() => {
    api
      .get<{ credit: number; displayName: string | null; username: string }>('/users/me')
      .then((me) => {
        if (me) {
  ...
  const handleSync = async () => {
    ...
    if (unsynced.length === 0) {
      Taro.showToast({ title: '已全部同步', icon: 'none' })
      return
    }
    try {
  ...
  return (
    <View className="profile-page">
      ...
      <View className="profile-menu">
        ...
        <View
          className="profile-menu__item"
          onClick={() => Taro.navigateTo({ url: '/pages/patient-select/index' })}
        >
          <Text>切换患者</Text>
        </View>
        <View
          className="profile-menu__item"
          onClick={() => Taro.navigateTo({ url: '/pages/pin-overview/index' })}
        >
          <Text>设备连接</Text>
        </View>
