...
interface FormDef { code: string; title: string; description?: string; fields: { length: number }; status: string }

export default function FormList() {
  const [forms, setForms] = useState<FormDef[]>([])

  useEffect(() => {
    api.get<FormDef[]>('/forms').then((r) => setForms((r || []).filter((f) => f.status === 'published'))).catch(() => {})
  }, [])
  ...
  return (
    <View className='page form-list-page'>
      ...
      {forms.map((f) => (
        <View key={f.code} className='form-card' onClick={() => Taro.navigateTo({ url: `/pages/form/index?code=${f.code}` })}>
