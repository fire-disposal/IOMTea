...
Page({
  ...
  onLoad(options) {
    this.setData({ trendData: getTrendData('spo2', 7), planId: options.planId || '' })
  },

  onValueInput(e) { this.setData({ value: e.detail.value }) },

  handleSave() {
    if (!this.data.value || Number(this.data.value) <= 0 || Number(this.data.value) > 100) {
      wx.showToast({ title: '请输入有效血氧值 (0-100)', icon: 'none' })
      return
    }
    this.setData({ saving: true })
    addLocalRecord({
      type: 'spo2',
      data: { percentage: Number(this.data.value) },
    ...
    syncUnsyncedRecords()
    this.setData({ trendData: getTrendData('spo2', 7) })
    wx.vibrateShort()
