...
import { ActionIcon, Badge, Button, Group, Modal, Paper, Select, Stack, Text, TextInput } from '@mantine/core'
import { notifications } from '@mantine/notifications'
import { IconPlus, IconTrash, IconLink } from '@tabler/icons-react'
import { trpc } from '../trpc'

const ROOM_TYPES = [
  { value: 'livingroom', label: '客厅' }, { value: 'bedroom', label: '卧室' },
  { value: 'kitchen', label: '厨房' }, { value: 'bathroom', label: '浴室' },
  { value: 'study', label: '书房' }, { value: 'entry', label: '玄关' },
  { value: 'corridor', label: '走廊' }, { value: 'balcony', label: '阳台' },
...
function autoPos(index: number): { x: number; y: number } {
  const positions = [
    { x: 0, y: 0 }, { x: 3, y: -3 }, { x: 3, y: 3 }, { x: -3, y: -3 },
    { x: -3, y: 3 }, { x: 6, y: 0 }, { x: -6, y: 0 }, { x: 0, y: 6 },
  ]
...
export function GraphEditorPage({ patientId }: { patientId: string }) {
  ...
  const upsert = trpc.homeGraph.upsert.useMutation({
    onSuccess: () => { utils.homeGraph.get.invalidate({ patientId }); notifications.show({ title: '已保存', message: '', color: 'green' }) },
    onError: (e: any) => notifications.show({ title: '保存失败', message: e.message, color: 'red' }),
  })
  ...
  const addRoom = () => {
    ...
    setRooms([...rooms, { id: `room-${Date.now()}`, name: '新房间', type: 'bedroom', x: pos.x, y: pos.y, connections: [] }])
  }
  ...
  if (graphQuery.isLoading) return <Text c="dimmed" ta="center" py="xl">加载中...</Text>
  ...
  return (
    <div style={{ height: 'calc(100vh - 60px)', display: 'flex', flexDirection: 'column' }}>
      <Group px="md" py="sm" bg="gray.0" justify="space-between" style={{ borderBottom: '1px solid #ddd' }}>
        ...
        <Group gap="xs">
          <Button size="xs" variant="light" leftSection={<IconPlus size={14} />} onClick={addRoom}>添加房间</Button>
          <Button size="xs" onClick={save} loading={upsert.isPending}>保存</Button>
        </Group>
      </Group>

      <div style={{ flex: 1, overflow: 'auto', padding: 16 }}>
        {rooms.length === 0 && <Text c="dimmed" ta="center" py="xl">暂无房间，点击"添加房间"开始</Text>}
          ...
          {rooms.map((room, i) => (
            <Paper key={room.id} p="md" withBorder radius="md">
              <Group justify="space-between" mb="sm">
                <Badge>房间 {i + 1}</Badge>
                <ActionIcon color="red" variant="subtle" onClick={() => removeRoom(room.id)}><IconTrash size={16} /></ActionIcon>
              </Group>
              <Group grow mb="sm">
              ...
              <Group gap="xs">
                {rooms.filter((r) => r.id !== room.id).map((r) => (
                  <Badge key={r.id} size="sm" variant={(room.connections ?? []).includes(r.id) ? 'filled' : 'outline'}
              ...
              <Select label="入口" data={rooms.map((r) => ({ value: r.id, label: r.name }))}
                value={entryRoomId ?? undefined} onChange={(v) => setEntryRoomId(v ?? null)}
                size="xs" mt="sm" clearable placeholder="选择入口房间" />
            </Paper>
          ))}
        </Stack>
      </div>
