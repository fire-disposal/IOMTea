...
export function GlobalMedications() {
  ...
  const patientsOnMeds = useMemo(
    () => new Set(allMeds.map((m: any) => m.patientId)).size,
    [allMeds],
  )
  ...
  const columns = useMemo(
    () => [
      ...
      columnHelper.accessor('dosage', {
        header: '剂量',
        cell: (info) => `${info.getValue()}${info.row.original.dosageUnit}`,
      }),
      columnHelper.accessor('route', {
        header: '途径',
        cell: (info) => (
          <Badge size="sm" variant="light" color="gray">
            {ROUTE_LABELS[info.getValue()] || info.getValue()}
          </Badge>
        ),
      }),
      columnHelper.accessor('status', {
        header: '状态',
        cell: (info) => (
          <Badge size="sm" color={STATUS_COLORS[info.getValue()] || 'gray'}>
      ...
      columnHelper.display({
        ...
        cell: (info) => (
          <Button
            ...
            onClick={() =>
              navigate({
                to: '/patients/$id/medications',
                params: { id: info.row.original.patientId },
              })
            }
          >
            查看
    ...
    <Container size="xl" py="xl">
      ...
      <SimpleGrid cols={2} mb="lg">
        <AccentPaper p="md" radius="md" withBorder color="matchaGreen">
          <Group>
            <ThemeIcon color="matchaGreen" variant="light">
              <IconPill size={20} />
            </ThemeIcon>
            <div>
