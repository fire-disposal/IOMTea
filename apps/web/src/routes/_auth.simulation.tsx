        ...
        <Grid.Col span={{ base: 12, lg: 5 }}>
          <Paper p="md" withBorder mb="md">
            <Group justify="space-between" mb="sm">
              ...
              <Button size="compact-sm" variant="light" color={creating ? 'green' : undefined}
                onClick={() => creating ? createSim.mutate({ profile: newProfile }) : setCreating(true)}
                loading={createSim.isPending} leftSection={<IconPlus size={14} />}>
                {creating ? '确认创建' : '从模板创建'}
              </Button>
            </Group>
            {creating && (
              <Select mb="sm" label="选择患者画像模板" data={PROFILES} value={newProfile}
                onChange={(v) => setNewProfile(v!)} description="模板仅提供初始配置，创建后可深度定制" />
            )}
          </Paper>

          {(simulations ?? []).length === 0 && !creating && (
            <Text c="dimmed" size="sm" ta="center" py="xl">暂无模拟配置，点击上方按钮从模板创建</Text>
          )}

          {(simulations ?? []).map((sim: any) => (
            <Paper key={sim.id} p="sm" withBorder mb="sm"
              style={{ cursor: 'pointer', borderLeft: selectedSimId === sim.id ? '3px solid var(--mantine-color-teal-5)' : undefined }}
              ...
              <Group justify="space-between" mb={4}>
                ...
                <Group gap="xs">
                  ...
                  <ActionIcon variant="subtle" color="red" size="xs"
                    onClick={(e) => { e.stopPropagation(); deleteSim.mutate({ id: sim.id }) }}>
                    <IconTrash size={12} />
                  </ActionIcon>
                </Group>
              </Group>
            ...
            <Stack>
              <Paper p="md" withBorder>
                  ...
                  <Table.Tbody>
                    {selected.metrics.map((m: any) => (
                      <Table.Tr key={m.name} opacity={m.enabled ? 1 : 0.4}>
                        <Table.Td>
                          <Switch size="xs" checked={m.enabled}
                            onChange={(e) => toggleMetric.mutate({ id: selected.id, metric: m.name, enabled: e.currentTarget.checked })} />
                        </Table.Td>
                        <Table.Td>
                          <Text size="xs" fw={m.enabled ? 600 : 400}>{METRIC_LABELS[m.name] ?? m.name}</Text>
                        </Table.Td>
                        <Table.Td>
                          <NumberInput size="xs" min={100} max={600000} w={80} hideControls
                            value={m.config.interval.min} disabled={!m.enabled}
                            onChange={(v) => handleMetricChange(m.name, 'intervalMin', Number(v))} />
                        </Table.Td>
                        <Table.Td>
                          <NumberInput size="xs" min={100} max={600000} w={80} hideControls
                            value={m.config.interval.max} disabled={!m.enabled}
                            onChange={(v) => handleMetricChange(m.name, 'intervalMax', Number(v))} />
                        </Table.Td>
                        <Table.Td>
                          <NumberInput size="xs" min={0} max={1} step={0.1} decimalScale={2} w={60} hideControls
                ...
                {selectedPatients.map((sp: any) => (
                  <Paper key={sp.patientId} p="sm" withBorder mb="xs" style={{ cursor: 'pointer' }}
                    onClick={() => { setDetailPatient(sp); setTimelineMinutes(10) }}>
                    <Group justify="space-between">
                      <Text size="sm" fw={500}>{sp.patientName}</Text>
                      <Group gap="xs">
                        <Badge size="xs" variant="light">tick: {sp.tickCount}</Badge>
                        <ActionIcon variant="subtle" color="red" size="xs"
                          onClick={(e) => { e.stopPropagation(); removePatients.mutate({ id: selected.id, patientIds: [sp.patientId] }) }}>
                          <IconTrash size={12} />
                        </ActionIcon>
                    ...
                    {(Object.entries(sp.lastValues ?? {}) as [string, number][]).slice(0, 4).map(([k, v]) => (
                      <Badge key={k} size="xs" variant="outline" mr={4} mt={4}>
                        {METRIC_LABELS[k] ?? k}: {typeof v === 'number' ? v.toFixed(1) : String(v)}
                      </Badge>
                    ))}
                  </Paper>
                ))}
