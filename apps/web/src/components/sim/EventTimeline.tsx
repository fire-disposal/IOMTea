...
export function EventTimeline({ patientId, minutes, onMinutesChange, title }: EventTimelineProps) {
  const { data, isLoading } = trpc.sim.events.useQuery(
    { patientId, minutes },
  ...
  const { lanes, timeRange, allMetrics } = useMemo(() => {
    ...
    return {
      lanes: keys.map((k) => [k, byMetric.get(k)!] as const),
      ...
      <Group gap={4}>
        {allMetrics.map((m) => (
          <Badge
            key={m}
            size="xs"
            variant={hiddenMetrics.has(m) ? 'outline' : 'filled'}
            color={hiddenMetrics.has(m) ? 'gray' : undefined}
            style={{
              cursor: 'pointer',
              background: hiddenMetrics.has(m) ? undefined : (METRIC_HEX[m] ?? '#666'),
            }}
              ...
              return (
                <Card
                  ...
                  style={{
                    ...
                    background: li % 2 === 0 ? 'transparent' : 'rgba(0,0,0,0.01)',
                  }}
                >
                      ...
                      return (
                        <Tooltip
                          ...
                          label={
                            <Box>
                              <Text size="xs">
                                {METRIC_LABELS[metric] ?? metric}: {p.value?.toFixed(1) ?? '-'}
                                {METRIC_UNITS[metric] ? ` ${METRIC_UNITS[metric]}` : ''}
                              </Text>
        ...
        {detailRecord && (
          <Stack gap="xs">
            <Group>
              ...
              <Badge>{METRIC_LABELS[detailRecord.metric] ?? detailRecord.metric}</Badge>
            </Group>
            <Group>
              <Text size="sm" fw={500}>
                值:
              ...
              <Text>
                {detailRecord.value?.toFixed(2) ?? '-'}
                {METRIC_UNITS[detailRecord.metric] ? ` ${METRIC_UNITS[detailRecord.metric]}` : ''}
              </Text>
            </Group>
            <Group>
              <Text size="sm" fw={500}>
              ...
              <Text size="sm">{new Date(detailRecord.recordedAt).toLocaleString()}</Text>
            </Group>
            <Text size="sm" fw={500}>
              原始记录:
            </Text>
