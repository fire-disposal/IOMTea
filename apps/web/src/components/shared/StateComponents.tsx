...
export function StateEmpty({ message = '暂无数据', action, actionLabel }: {
  message?: string
  action?: () => void
  actionLabel?: string
}) {
  return (
    <Center py={60}>
      <Stack align="center" gap="md">
        ...
        {action && <Button variant="light" onClick={action}>{actionLabel}</Button>}
      </Stack>
    </Center>
  )
}

export function StateError({ message = '加载失败', onRetry }: { message?: string; onRetry?: () => void }) {
  return (
    <Center py={60}>
      <Stack align="center" gap="md">
