...
export function DeviceListPage() {
  ...
  const create = trpc.device.create.useMutation({
    onSuccess: () => {
      ...
      setCreateOpen(false)
      resetForm()
      notifications.show({ title: '创建成功', message: '设备已添加', color: 'green' })
    },
    onError: (err: any) =>
  ...
  const update = trpc.device.update.useMutation({
    onSuccess: () => {
      utils.device.list.invalidate()
      notifications.show({ title: '更新成功', message: '设备状态已切换', color: 'green' })
    },
    onError: (err: any) =>
      notifications.show({ title: '操作失败', message: err.message, color: 'red' }),
  })
  const del = trpc.device.delete.useMutation({
    onSuccess: () => {
      ...
      notifications.show({ title: '已删除', message: '设备已移除', color: 'orange' })
    },
    onError: (err: any) =>
      notifications.show({ title: '操作失败', message: err.message, color: 'red' }),
  })
  ...
  return (
    <Container size="xl" py="md">
          ...
          {data?.length === 0 ? (
            ...
            data?.map((d: any) => (
              <Table.Tr key={d.id}>
                ...
                <Table.Td>
                  <Badge variant="light">{typeLabels[d.deviceType] || d.deviceType}</Badge>
                </Table.Td>
                <Table.Td>
      ...
      >
        ...
        <Select
          label="设备类型"
          data={['mattress', 'vision', 'imu', 'generic', 'simulator', 'custom']}
          value={form.deviceType}
          onChange={(v) => setForm({ ...form, deviceType: (v as DeviceType) || 'generic' })}
          mb="sm"
        />
        <Button fullWidth onClick={handleCreate} loading={create.isPending}>
          创建
        </Button>
      </Modal>
    </Container>
