...
export function PatientMedications() {
  ...
  const schedulesQueries = trpc.useQueries(
    (t) => meds.data?.map((m: any) => t.medication.schedules({ medicationId: m.id })) ?? [],
  )
  ...
  const adherenceQueries = trpc.useQueries(
    (t) =>
                    ...
                    <Table.Td>
                      <Group gap={4}>
                        ...
                        <ActionIcon
                          ...
                          variant="subtle"
                          color="gray"
                          onClick={() => setSelectedMedId(selectedMedId === m.id ? null : m.id)}
                        >
                          <Text>📊</Text>
                        </ActionIcon>
                        <ActionIcon size="sm" variant="subtle" onClick={() => openEdit(m)}>
                          <Text>✏️</Text>
                        </ActionIcon>
                        <ActionIcon
                          size="sm"
          ...
          <form.Field name="frequency">
            {(f) => (
              <Select
                ...
                data={frequencies}
                value={f.state.value}
                onChange={(v) => f.handleChange(v ?? '每日一次')}
              />
            )}
          </form.Field>
          <form.Field name="route">
            {(f) => (
              <Select
                ...
                value={f.state.value}
                onChange={(v) => f.handleChange(v ?? 'oral')}
              />
            )}
          </form.Field>
