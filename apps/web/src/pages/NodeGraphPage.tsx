...
const roomTypeOptions = [
  ...
  { value: 'kitchen', label: '厨房' }, { value: 'bathroom', label: '卫生间' },
  { value: 'study', label: '书房' }, { value: 'corridor', label: '走廊' },
  { value: 'entry', label: '玄关' }, { value: 'balcony', label: '阳台' },
  { value: 'storage', label: '储物间' }, { value: 'dining', label: '餐厅' },
]
...
interface RoomModel {
  ...
  patientId?: string
  patientName?: string
  devices: { id: string; label?: string; serialNumber?: string; pin?: string; deviceType?: string; status?: string }[]
}
...
export function NodeGraphPage() {
  ...
  const [editingRoom, setEditingRoom] = useState<{ id: string; name: string; type: string } | null>(null)
  ...
  const handleCreateRoom = useCallback(() => {
    ...
    rooms.push({
      id, name: newRoomName || '新房间', type: newRoomType,
      x: 0, y: 0, connections: [], devices: [],
    })
  ...
  const handleSaveGraph = useCallback(() => {
    saveGraph.mutate({ graph: { rooms: rooms.map((r) => ({
      id: r.id, name: r.name, type: r.type, x: r.x, y: r.y,
      connections: r.connections, patientId: r.patientId,
    })) } } as any)
  }, [rooms, saveGraph])
  ...
  if (isLoading) {
    return <Box style={{ height: 'calc(100vh - 112px)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
  ...
  return (
    <Box style={{ height: 'calc(100vh - 112px)', display: 'flex', position: 'relative' }}>
      <Box style={{ flex: 1, display: 'flex', flexDirection: 'column', background: '#f8faf9', overflow: 'auto' }}>
        <Box p="sm" style={{ borderBottom: '1px solid var(--mantine-color-gray-2)', display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
          <Group gap="xs">
            <Button size="xs" variant="light" leftSection={<IconArrowsMaximize size={14} />}
              onClick={() => setScale((s) => Math.min(s + 0.1, 2))}>放大</Button>
            <Button size="xs" variant="light" leftSection={<IconZoomOut size={14} />}
              onClick={() => setScale((s) => Math.max(s - 0.1, 0.5))}>缩小</Button>
            <Button size="xs" variant="light" leftSection={<IconLayoutGrid size={14} />}
              onClick={() => setScale(1)}>重置</Button>
          </Group>
          <Box style={{ flex: 1 }} />
          <TextInput size="xs" placeholder="搜索房间..." value={searchText}
            onChange={(e) => setSearchText(e.currentTarget.value)} />
          <Button size="xs" variant="filled" onClick={handleSaveGraph} loading={saveGraph.isPending}>保存布局</Button>
        </Box>

        <Box p="md" style={{
          flex: 1, transform: `scale(${scale})`, transformOrigin: 'top left',
          display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))',
          gap: 16, alignContent: 'start', transition: 'transform 0.15s',
        }}>
          {rooms.filter((r) => !searchText || r.name.toLowerCase().includes(searchText.toLowerCase())).map((room) => (
            <Box key={room.id} style={{ position: 'relative' }}>
              ...
              {selectedRoom === room.id && (
                <Group gap={4} mt={4} style={{ position: 'absolute', top: '100%', left: 0, zIndex: 10 }}>
                  <Button size="xs" variant="subtle" color="gray"
                    onClick={() => setEditingRoom({ id: room.id, name: room.name, type: room.type })}>编辑</Button>
                  <Button size="xs" variant="subtle" color="red"
                    onClick={() => handleDeleteRoom(room.id)}>删除</Button>
                </Group>
              )}
            </Box>
          ))}
      ...
      <Modal opened={creatingRoom} onClose={() => setCreatingRoom(false)} title="创建房间" size="sm">
        <Stack gap="sm">
          <TextInput label="房间名称" value={newRoomName}
            onChange={(e) => setNewRoomName(e.currentTarget.value)} placeholder="例如：主卧" />
          <Select label="房间类型" data={roomTypeOptions} value={newRoomType}
      ...
      <Modal opened={!!editingRoom} onClose={() => setEditingRoom(null)} title="编辑房间" size="sm">
        <Stack gap="sm">
          <TextInput label="房间名称" value={editingRoom?.name ?? ''}
            onChange={(e) => setEditingRoom((prev) => prev ? { ...prev, name: e.currentTarget.value } : null)} />
          <Select label="房间类型" data={roomTypeOptions} value={editingRoom?.type ?? 'bedroom'}
            onChange={(v) => setEditingRoom((prev) => prev ? { ...prev, type: v ?? 'bedroom' } : null)} />
