...
export function PatientListPage() {
  ...
  const create = trpc.patient.create.useMutation({
    onSuccess: () => {
      ...
      setCreateOpen(false)
      resetForm()
      notifications.show({ title: '创建成功', message: '患者已添加', color: 'green' })
    },
    onError: (err: any) =>
  ...
  const update = trpc.patient.update.useMutation({
    onSuccess: () => {
      utils.patient.list.invalidate()
      setEditId(null)
      notifications.show({ title: '更新成功', message: '患者信息已更新', color: 'green' })
    },
    onError: (err: any) =>
      notifications.show({ title: '操作失败', message: err.message, color: 'red' }),
  })
  const del = trpc.patient.delete.useMutation({
    onSuccess: () => {
      ...
      notifications.show({ title: '已删除', message: '患者已移除', color: 'orange' })
    },
    onError: (err: any) =>
      notifications.show({ title: '操作失败', message: err.message, color: 'red' }),
  })
    ...
    <Container size="xl" py="md">
      ...
      <Table striped highlightOnHover>
        <Table.Thead>
          <Table.Tr>
            ...
            <Table.Th>房间</Table.Th>
            <Table.Th>床位</Table.Th>
      ...
      >
        ...
        <TextInput
          ...
          onChange={(e) => setForm({ ...form, bedNumber: e.currentTarget.value })}
          mb="sm"
        />
      ...
      >
        ...
        <TextInput
          ...
          mb="sm"
        />
        <Button fullWidth onClick={handleUpdate} loading={update.isPending}>
          保存
        </Button>
      </Modal>
    </Container>
