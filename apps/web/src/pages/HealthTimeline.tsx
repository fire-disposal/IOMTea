import { Container, Select, Group, MultiSelect, Paper, Skeleton } from '@mantine/core'
...
export function HealthTimeline() {
  ...
  const { data: rawData, isLoading } = useGet<{ rows: { recordedAt: number; value: unknown; metric?: string }[] }>(
        ...
        {isLoading ? <Skeleton height={300} /> : (
          <GanttTimeline
            data={(rawData?.rows ?? []).map((r) => ({
              ...
              displayName: metrics?.find((m) => m.metric === (r.metric || selected[0]))?.displayName ?? (r.metric || ''),
            }))}
            height={350}
          />
        )}
