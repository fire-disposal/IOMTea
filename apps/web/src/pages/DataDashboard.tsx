import { Container, Title, Paper, Select, Group, Box, Text, ThemeIcon } from '@mantine/core'
import { IconChartLine } from '@tabler/icons-react'
import { useEffect, useState } from 'react'
import { http } from '../api/client'

interface MetricInfo {
  metric: string; displayName: string; unit: string; defaultChart: string; category: string
}
...
export function DataDashboard() {
  ...
  useEffect(() => {
    ...
    }).then((res) => { setTrend((res.data as any)?.rows ?? []) }).catch(() => {})
  }, [selected])
  ...
  return (
    <Container py="md">
      <Group justify="space-between" mb="md">
        <Group gap="xs">
          <ThemeIcon size="sm" variant="light"><IconChartLine size={14} /></ThemeIcon>
