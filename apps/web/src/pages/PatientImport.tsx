  ...
  const handleImport = () => {
    ...
    bulkCreate.mutate({
      ...
      patients: validRows.map((r) => ({
        ...
        bloodType: r.blood_type as 'A' | 'B' | 'AB' | 'O' | undefined,
  ...
  return (
    <Modal opened={opened} onClose={handleClose} title="批量导入患者" size="xl">
      <Stepper active={step} onStepClick={setStep}>
        <Stepper.Step label="上传文件" description="CSV 格式">
          <Text size="xs" c="dimmed" mb="sm">
            ...
            <Text span ff="monospace">
              name,phone,gender,birth_date,height_cm,weight_kg,blood_type,address,emergency_contact,emergency_phone
            </Text>
            <br />
            必填:{' '}
            <Text span fw={500}>
            ...
            。性别取值: male/female/other。日期格式: YYYY-MM-DD。
            <br />
            示例行:{' '}
            <Text span ff="monospace">
              张三,13800138000,male,1980-05-15,170,65,A,北京市东城区,张四,13900139000
            </Text>
          </Text>
          <FileInput
            label="选择 CSV 文件"
        ...
        <Stepper.Step
          label="预览校验"
          description={`${validRows.length} 有效 / ${invalidRows.length} 无效`}
        >
          <Table striped>
            <Table.Thead>
              <Table.Tr>
          ...
          <Button
            ...
            disabled={!defaultPassword || validRows.length === 0}
          >
