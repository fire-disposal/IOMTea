...
import { createEngine, startEngine, stopEngine, getEngine, getEngineStatus, listEngines } from '../engine'
...
export const simulationRouter = router({
  ...
  listSimulations: protectedProcedure
    ...
    .query(async ({ ctx }) => {
      ...
      for (const engine of engines) {
        const [patient] = await ctx.db.select({ name: patients.name }).from(patients).where(eq(patients.id, engine.patientDbId)).limit(1).catch(() => [])
        results.push({
  ...
  delete: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await ctx.db.delete(patients).where(eq(patients.id, input.patientId)).catch(() => {})
      return { success: true }
    }),
})