...
export function generateHeartRate(baseline: { mean: number; std: number }, hour: number, _prev?: number): number {
  return Math.round(gaussian(baseline.mean + circadianFactor(hour, -3, 5), baseline.std))
}

export function generateSpO2(baseline: { mean: number; std: number }, hour: number, _prev?: number): number {
  const dip = hour >= 2 && hour <= 5 ? -1.5 : 0
  return Number((gaussian(baseline.mean + dip, baseline.std)).toFixed(1))
}

export function generateTemperature(baseline: { mean: number; std: number }, hour: number, _prev?: number): number {
  return Number((gaussian(baseline.mean + circadianFactor(hour, -0.5, 0.3), baseline.std)).toFixed(1))
}

export function generateSystolicBp(baseline: { mean: number; std: number }, hour: number, _prev?: number): number {
  return Math.round(gaussian(baseline.mean + circadianFactor(hour, -5, 8), baseline.std))
}

export function generateDiastolicBp(baseline: { mean: number; std: number }, hour: number, _prev?: number): number {
  return Math.round(gaussian(baseline.mean + circadianFactor(hour, -3, 5), baseline.std))
}

export function generateGlucose(baseline: { mean: number; std: number }, hour: number, _prev?: number): number {
...
export function generateRespiratoryRate(baseline: { mean: number; std: number }, _hour: number, _prev?: number): number {
