...
import { plans, planCompletions, creditTransactions } from '../core/db/schema/plan'
import { events, users } from '../core/db/schema'
import { planCreateSchema, planUpdateSchema, planCompleteSchema, planSchema } from '@iomtea/shared-types'
...
plansApp.openapi(updateRoute, async (c) => {
  ...
  const [row] = await db.update(plans).set(c.req.valid('json') as any).where(eq(plans.id, id)).returning()
...
plansApp.openapi(deleteRoute, async (c) => {
  const id = c.req.param('id')
  await db.update(plans).set({ status: 'archived' } as any).where(eq(plans.id, id))
...
plansApp.openapi(todayRoute, async (c) => {
  ...
  const doneIds = (await db.select({ planId: planCompletions.planId })
...
plansApp.openapi(completeRoute, async (c) => {
  ...
  const [completion] = await db.insert(planCompletions).values({
    planId,
  ...
  if (plan.rewardCredits > 0) {
    ...
    if (uid) {
      ...
      await db.update(users).set({ credit: users.credit ?? 0 + plan.rewardCredits } as any).where(eq(users.id, uid))
    }
  }
...
const completionsRoute = createRoute({
  method: 'get', path: '/:id/completions',
...
plansApp.openapi(completionsRoute, async (c) => {
  ...
  const rows = await db.select().from(planCompletions).where(and(...conds)).orderBy(planCompletions.completedAt)
  return c.json(rows)
})
