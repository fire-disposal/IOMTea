...
import type { AppointmentStatus, AppointmentType, FollowupType } from '../db/schema/enums'
...
export async function listAllAppointments(
  db: DbClient,
  opts?: { status?: AppointmentStatus; patientIds?: string[] },
) {
  const conditions = []
  if (opts?.status) conditions.push(eq(appointments.status, opts.status))
  if (opts?.patientIds?.length) conditions.push(inArray(appointments.patientId, opts.patientIds))
  return db
    .select()
    .from(appointments)
...
export async function updateAppointment(
  ...
  const { id: _id, scheduledAt, ...rest } = input as { id?: string } & typeof input
...
export async function listFollowups(db: DbClient, patientId: string) {
  return db
    ...
    .where(eq(followupRecords.patientId, patientId))
    .orderBy(desc(followupRecords.conductedAt))
}
