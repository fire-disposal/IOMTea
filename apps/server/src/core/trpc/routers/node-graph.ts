...
export const nodeGraphRouter = router({
  getGraph: protectedProcedure
    .query(async ({ ctx }) => {
      ...
      const allPatients = await ctx.db.select({
        id: patients.id, name: patients.name, tags: patients.tags,
      }).from(patients).where(eq(patients.userId, userId))

      const allDevices = await ctx.db.select({
        id: devices.id, serialNumber: devices.serialNumber,
        deviceType: devices.deviceType, status: devices.status,
        roomId: devices.roomId, patientId: devices.patientId,
        lastSeenAt: devices.lastSeenAt, tags: devices.tags,
      }).from(devices)

      const allPins = await ctx.db.select({
        pin: usersPin.pin, label: usersPin.label,
        nickname: usersPin.nickname, roomId: usersPin.roomId,
        isVirtual: usersPin.isVirtual, lastSeenAt: usersPin.lastSeenAt,
        generatorConfig: usersPin.generatorConfig,
      }).from(usersPin).where(eq(usersPin.userId, userId))
          ...
          for (const r of graph.rooms) {
            ...
            const roomDevices = allDevices
              ...
              .map((d: any) => ({
                ...
                status: d.status, lastSeenAt: d.lastSeenAt,
              }))

            const roomPins = allPins
              .filter((p: any) => p.roomId === r.id)
              .map((p: any) => ({
                id: p.pin, label: p.label ?? p.nickname ?? p.pin,
      ...
      const unassignedDevices = [
        ...allDevices.filter((d: any) => !d.roomId).map((d: any) => ({
          id: d.id, serialNumber: d.serialNumber, deviceType: d.deviceType,
          status: d.status, lastSeenAt: d.lastSeenAt, patientId: d.patientId,
        })),
        ...allPins.filter((p: any) => !p.roomId).map((p: any) => ({
          id: p.pin, label: p.label ?? p.nickname ?? p.pin,
          deviceType: p.isVirtual ? 'virtual' : 'pin',
          status: p.lastSeenAt ? 'active' : 'inactive',
          lastSeenAt: p.lastSeenAt,
      ...
      const latestEvents = await ctx.db.select({
        patientId: events.patientId, metric: events.metric,
        value: events.value, unit: events.unit,
        recordedAt: events.recordedAt,
      }).from(events).where(
        eq(events.kind, 'observation' as any),
      ).orderBy(desc(events.recordedAt)).limit(200)
      ...
      for (const e of latestEvents) {
        ...
        if (!seenEvents.has(key)) {
          ...
          patientEvents.get(e.patientId)!.push({
            metric: e.metric, value: e.value, unit: e.unit, recordedAt: e.recordedAt,
          })
  ...
  saveGraph: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const pts = await ctx.db.select({
        id: patients.id, tags: patients.tags,
      }).from(patients).where(eq(patients.userId, userId))

      for (const p of pts) {
        const tags = (p.tags as Record<string, unknown>) || {}
        const graph = (tags.homeGraph as any) || { rooms: [], entryRoomId: null, personLocation: null }
        ...
        await ctx.db.update(patients).set({ tags: newTags as any }).where(eq(patients.id, p.id))
      }
  ...
  assignDevice: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      const isUuid = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(input.deviceId)

      if (isUuid) {
        const [dev] = await ctx.db.select({ id: devices.id }).from(devices).where(eq(devices.id, input.deviceId)).limit(1)
        if (!dev) throw new TRPCError({ code: 'NOT_FOUND', message: '设备不存在' })
        await ctx.db.update(devices).set({ roomId: input.roomId }).where(eq(devices.id, input.deviceId))
      } else {
        await ctx.db.update(usersPin).set({ roomId: input.roomId }).where(eq(usersPin.pin, input.deviceId))
      }
  ...
  deleteRoom: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      for (const p of pts) {
        ...
        const updatedConnections = (graph.rooms || []).map((r: any) => ({
          ...r,
      ...
      await ctx.db.update(devices).set({ roomId: null as any }).where(eq(devices.roomId, input.roomId))
      await ctx.db.update(usersPin).set({ roomId: null as any }).where(eq(usersPin.roomId, input.roomId))
