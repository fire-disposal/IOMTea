...
export const simulatorRouter = router({
  ...
  pause: protectedProcedure.input(z.object({ wardId: z.string() })).mutation(({ input }) => {
    const ok = pauseWard(input.wardId)
    return { success: ok }
  }),

  resume: protectedProcedure.input(z.object({ wardId: z.string() })).mutation(({ input }) => {
  ...
  status: publicProcedure.input(z.object({ wardId: z.string() }).optional()).query(({ input }) => {
    if (input?.wardId) {
